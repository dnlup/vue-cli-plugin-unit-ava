...
const DEFAULT_OPTIONS = {
  ...
  babel: false,
  require: path.join(rootPath, 'node_modules/@dnlup/vue-cli-plugin-unit-ava/setup.js')
}
...
function exit (message) {
  console.error(`\n${require('ava/lib/chalk').get().red(figures.cross)} ${message}`)
...
exports.run = () => { // eslint-disable-line complexity
  ...
  try {
    const {
      pluginOptions: {
        '@dnlup/vue-cli-plugin-unit-ava': opts
      }
    } = require(path.join(rootPath, 'vue.config.js'))
    const keys = [
      'files',
      'sources',
      'match',
 ...
 `, {
  ...
  const chalk = require('ava/lib/chalk').set({ enabled: cli.flags.color })
  ...
  const normalizeExtensions = require('ava/lib/extensions')
